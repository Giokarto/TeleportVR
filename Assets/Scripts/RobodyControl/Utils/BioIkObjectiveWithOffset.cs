...
namespace RobodyControl
{
    ...
    /// limit the rotation Objective's weight, giving priority to the position constraint when orientation 
    /// is in unreachable configurations.
    /// </summary>
    public class BioIkObjectiveWithOffset : MonoBehaviour
    {

        [System.Serializable]
        public class Position
        {
            [Tooltip("If position offset should be applied in the negative y-direction of controller up")]
        ...
        [Header("Read only values")] [Tooltip("Current orientation offset, overwritten at startup")]
        public Vector3 orientationOffset;

        [Tooltip("Computed error values")] [SerializeField]
        float errorR, errorP, weight;
        ...
        void Update()
        {
            ...
            {
                float errorR = Quaternion.Angle(orientation.target.rotation, transform.rotation) * Mathf.Deg2Rad /
                               Mathf.PI;
