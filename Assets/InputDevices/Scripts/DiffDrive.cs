...
namespace DiffDrive
{
    public class DiffDrive : Singleton<DiffDrive>
    {
...
//#else
            ...
            float steeringAngle = rightJoystick.x;

            var angular = rightJoystick.x;
            var linear = leftJoystick.y;

            var l = (float) (linear - angular/ 2.0);
            var r = (float)(linear + angular / 2.0);
            //// TODO clean up
            //var minPwm = 5;
            //var maxPwm = 30;
            //var lPwm = Mathf.Abs(l) * (maxPwm - minPwm) + minPwm;
            //var rPwm = Mathf.Abs(r) * (maxPwm - minPwm) + minPwm;

            //l = Mathf.Sign(l) * lPwm;
            //r = Mathf.Sign(r) * rPwm;
            _output = new Vector2(l, r);
            //Debug.Log("output: " + _output);
            /*

            // keeping "left"=backward and "right"=forward variables from the pedals implementation
            float left = (leftJoystick.y > 0) ? 0 : Mathf.Abs(leftJoystick.y);
            float right = (leftJoystick.y < 0) ? 0 : Mathf.Abs(leftJoystick.y);

            leftWindow.Add(left);
            rightWindow.Add(right);

            Debug.Log("joystick: " + left + "\t" + right + "\t" + steeringAngle);
